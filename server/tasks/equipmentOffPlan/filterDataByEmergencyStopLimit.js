// Фильтровать данные по лимиту аварийных остановок (оставить узлы, которые не требуют ремонта)
...
module.exports = function (data) {
    ...
    // - В третьем цикле оставить только коды, не соответствующие лимиту
        ...
        const filteredObj = d[key]['data'].filter((item) => {
            ...
            Object.keys(item.nodes).forEach((node) => {
                ...
                if (item.nodes[node].amount >= LIMIT_NUMBER_EMERGENCY_STOPS[key]) {
                    // Удалить код простоя, если его количество больше или равно допустимого лимита
                    delete item.nodes[node]
                } else {
                    // Данные рассчитываются по оставшимся узлам
